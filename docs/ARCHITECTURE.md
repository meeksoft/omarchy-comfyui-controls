...
2. A controller helper will own server lifecycle operations and emit structured
   JSON responses.
3. A ComfyUI client will read health, queue, progress, history, and preview
...
5. If the port is free, launch one managed process and record its ownership.
